...
def add_columns(df, start_date, end_date, n, path):
    ...
    # reading in postcode data /before data
...
def main():
    ...
    s3 = boto3.resource('s3', aws_access_key_id='aws_secret_id', aws_secret_access_key='=aws_secret_access_key')
    null_df = df[df['Delivery Date'].isnull()]
    #adding data cleaning
    file_name = (f'order_data_{dt.datetime.today().strftime("%Y%m%d_%H%M")}.csv')
    print("Data frame")
    #df['Order Date'] = df['Order Date'].astype('datetime64[s]')
    df['Order Date'] = pd.to_datetime(df['Order Date']).dt.date
    #df['Dispatched Date'] = df['Dispatched Date'].astype('datetime64[s]')
    df['Dispatched Date'] = pd.to_datetime(df['Dispatched Date']).dt.date
    #df['Delivery Date'] = df['Delivery Date'].astype('datetime64[s]')
    df['Delivery Date'] = pd.to_datetime(df['Delivery Date']).dt.date
    df['Delivery Postcode'] = df['Delivery Postcode'].str.replace('%20', '', regex=True)
    df.to_csv(f'{path}/{file_name}', index=False)
    null_order = 'null_order_data.csv'
    #uploding on s3
    s3.Bucket('tootbrushdata').upload_file(Filename=file_name, Key=file_name)
    print(f'Saved file {file_name}')
    print(f'Saved file {file_name} to {path}')
    ###adding data cleansing method
    #os.remove(file_name)
    print(f"{file_name} is deleted from syste directory and uploaded to s3")
    null_df.to_csv('null_order_data.csv', index=False)
    null_df.to_csv(f'{path}/null_order_data.csv', index=False)
    #null order data csv file
    null_df.to_csv(f'{path}/{null_order}', index=False)
    #uploading on s3
    s3.Bucket('tootbrushdata').upload_file(Filename='null_order_data.csv', Key='null_order_data.csv')
    print(f"Saved file {null_order} to {path}")
    #delete the files after uploading to s3 from system
    os.remove(file_name)
    os.remove(null_order)
    print(f"{null_order} is deleted from syste directory and uploaded to s3")
    print(f"Saved file {file_name}")
